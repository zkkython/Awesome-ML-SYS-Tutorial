...
注：上面描述的single controller和SPMD模式只是经验上的practice，但实际上他们描述的是2个重点：其中single controller主要关注于控制流是否单点；而SPMD模式更关注的是分布式场景下数据执行流。比如，即使是single controller，也是可以使用SPMD模式进行数据流执行的。
